...
- feat: smarter accounting behind the scenes. The heat pump now tells the integration what it is doing (heating, cooling, hot water, defrost), and energy is booked to the right mode — no more heating credit while it is making hot water or cooling the floor. Defrost gets its own energy meter, since during a defrost cycle heat flows out of your house rather than in, and a pump-only circulation break counts as nothing
- fix: heating power and COP no longer disappear on v3.4 units during normal operation. The firmware sometimes reports compressor frequency 0 for a whole poll while the compressor is actually running; the integration now cross-checks the unit's electrical draw against the water-side heat balance (flow × temperature rise) before concluding the compressor is off. Genuine pump-only breaks stay suppressed
- i18n: all new sensors are named in English, German and Russian, and sort nicely together on the device page
- diagnostics: the diagnostics download now shows the current mode, all three COPs and the energy counters, making support questions faster to answer
...
## 0.6.15 - 2026-09-18
- fix(changelog): reworded 0.6.14 entry, no code change
## 0.6.14 - 2026-09-18
...
- feat(curve-image): friendlier legend — fixed 2x2 grid (curve / fixed / live dot / band preview) with bold active mode + summary line incl. R04/R05; X tick at AT=0 shows plain 0C; RU pair is Start/Stop heating (no more Idle/waiting wording), live pill is just the target number (target may drive inlet, outlet or room temp)
- perf(curve-image): feDropShadow raster filter removed (blurred text on phone GPUs when scaled down) — floating labels use a vector paint-order halo; system-ui font stack; threshold/summary type 14->15; pill widths measure cyrillic wider than latin so RU text no longer overflows
- chore(numbers): number entities stay uniform sliders (box-stepper experiment reverted — mixed rows looked inconsistent; exact phone input goes through number.set_value / Assist)
## 0.6.4 - 2026-09-08
...
- feat(alias): alias_switch JSON format exposes a plain register as a normal-mode switch — H22 silent enable is one slider (switch.foxair_silent_mode) in both modes; 1016 bit1 proven manual-override flag (not live status), H22 select stays expert-only
- fix(silent): silent switch actually drives the pump now (was writing the 1016 status word while H22 governs) + optimistic state so the slider reacts instantly instead of waiting up to 5 min for the rare poll
- feat(s-device): S01 contacts moved to their own FoxAir Switches [S] sub-device (was main-device noise); new blocks.labels gate fails the build on unlabeled blocks
- feat(normal-mode): H05/H07/H18/H20/H21/H25/H28/H32/H37/H40 + H36 user settings visible without expert mode (H30/H43 excluded); config-driven popular_addrs enables them by default (H32 was appearing deactivated)
- fix(i18n): H07 is Display/Remote control (was Master/Slave, EN name even said Dry Contact); H25 unabbreviated + typo/inlet fixes (EN/DE/RU)
- fix(ha-deps): via_device replaced with via_device_id (registry lookup, removal 2027.8); registry scans use async_entries_for_config_entry (removal 2027.9); both fail the build if reintroduced
- chore: CHANGELOG bullets are - not |- (pipe opens a table row on GitHub/HACS); validate enforces pipe-free lines + top section == VERSION
...
## 0.5.3 - 2026-09-02
- feat(fw-gate): register 1334 (SG Ready modes 1/2), 1016 (manual defrost / silent flag / manual heat), and 1018 (power-off-memory autostart) are firmware-gated to Firmware >= 3.3. Firmware version is auto-detected from register 2104 (raw value = major*10+minor, e.g. 33 = v3.3); register 2104 is now polled on quick tier and exposed as `sensor.foxair_2104` (Main Software Version). On firmware < 3.3 (or when not yet detected), the three SG/feature registers are silently hidden from entity creation, polling, and writes; they appear automatically when the integration reloads after a firmware upgrade. `min_firmware` field added to `foxair_config.json` overrides + `foxair_metadata.json`, enforced in coordinator `_tier_addrs` (skip), `_validate_write` (block), and all platform `async_setup_entry` (skip creation) + `_cleanup_orphaned_entities` (remove stale)
