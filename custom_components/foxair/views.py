...
class FoxAirCurveSvgView(HomeAssistantView):
    ...
    async def get(self, request):
        ...
        foxair_data = hass.data.get("foxair", {})

        if not entry_id or entry_id not in foxair_data:
            return web.Response(
                body=b'<svg xmlns="http://www.w3.org/2000/svg" width="800" height="460"><rect width="100%" height="100%" fill="#0f172a"/><text x="400" y="230" text-anchor="middle" fill="#94a3b8" font-family="sans-serif" font-size="16">No entry_id or entry not found</text></svg>',
                content_type="image/svg+xml",
                headers={"Cache-Control": "no-cache, max-age=30"}
            )

        coord = foxair_data[entry_id]
        return await self._render_svg(coord, entry_id)

    async def _render_svg(self, coord, entry_id):
        slope = 0.6
        offset = 0
        r10, r11 = 20, 60
        r31, r34 = 60, 35
        ...
        def x_at(atv):
            return pad_l + (atv + 20) / 40 * plot_w

        def y_flow(f):
            return pad_t + (70 - f) / 60 * plot_h
...
<line x1="{pad_l}" y1="{H-pad_b-legend_h}" x2="{W-pad_r}" y2="{H-pad_b-legend_h}" stroke="#334155"/>'''
        ...
        if fixed is not None:
            svg += f'<line x1="{pad_l}" y1="{fixed_y:.1f}" x2="{W-pad_r}" y2="{fixed_y:.1f}" stroke="#f59e0b" stroke-dasharray="6 4" opacity="0.9"/><text x="{pad_l+8}" y="{fixed_y-6:.0f}" fill="#fbbf24" font-size="11">Fixed R02 {fixed:.1f}°C</text>'
...
class FoxAirCurvePanelView(HomeAssistantView):
    ...
    async def get(self, request):
        ...
        if not entry_id or entry_id not in foxair_data:
            # List available entries for selection
            entries_html = ""
            for eid, coord in foxair_data.items():
                prefix = coord.entry.data.get("name_prefix", "foxair")
                slave = coord.entry.data.get("slave", 1)
                entries_html += f'<li><a href="/api/foxair/heating-curve-panel?entry_id={eid}">{prefix} (slave {slave})</a></li>'
...
<ul>{entries_html or "<li>No pumps configured</li>"}</ul>
        ...
        html = f"""<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>FoxAir Heating Curve — {prefix} (slave {slave})</title>
...
<div class="wrap">
<h1>FoxAir Heating Curve — {prefix} (slave {slave}) <span class="muted">— auto panel, no Lovelace edit</span></h1>
<div class="card"><img src="/api/foxair/heating_curve.svg?entry_id={entry_id}&v={at}" alt="Heating curve"></div>
...
<p><a href="/api/foxair/heating_curve.svg?entry_id={entry_id}" target="_blank" style="color:#38bdf8">Open SVG standalone</a></p>
</div>
</div>"""
        return web.Response(body=html.encode("utf-8"), content_type="text/html")